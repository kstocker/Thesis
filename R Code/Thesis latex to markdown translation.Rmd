...
\begin{figure}[htbp]
\includegraphics[scale=.5, center]{LinearZ_NY.png}
\caption{Plot of the reconstructed $Z_{t}$ parameter for measles data in New York City from 1920-1940 using global linear regression, as described in the susceptible reconstruction section above.}
\label{fig:Zlinear}
\end{figure}

```{r Zlinear, echo=FALSE, fig.cap="Plot of the reconstructed $Z_{t}$ parameter for measles data in New York City from 1920-1940 using global linear regression, as described in the susceptible reconstruction section above."}
...
In order to address the temporal variation in the rate at which cases are reported, it is necessary to perform localized linear regression. To do this, I will split the 
