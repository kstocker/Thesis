...
#possible error: city$___ might not work... try changing to city[[1]]$___. 
...
gaussian_regression <- function(city, scale){
  cumbirths <- cumsum(city[[1]]$rec)
  cumcases <- as.numeric(cumsum(city[[1]]$cases))
...
bayes_data <- function(city){
  return(list(N=length(city[[1]]$number), P=as.integer(city[[1]]$pop), I=as.integer(city[[1]]$I_gaus),
              Z=as.integer(city[[1]]$Z_gaus), Zmin=as.integer(abs(min(city[[1]]$Z_gaus)))))
}
...
As an example, let's look at measles data from pre-vaccination era New York City,  from 1920-1940 [@tycho]. Figure \ref{fig:Zlinear} is a graph of the reconstructed susceptible dynamics, obtained as previously described using global linear regression. It is evident from figure \ref{fig:Zlinear} that the Z dynamics suffer from local shifts away from the mean of zero. This indicates that the previously held assumption of constant reporting rate, $\rho$, has been violated. 



```{r Zlinear, echo=FALSE, fig.cap="Plot of the reconstructed Z parameter for measles data in New York City from 1920-1940 using global linear regression, as described in the susceptible reconstruction section above."}

newyork <- read_in_city("NEW YORK", naremove=FALSE)

newyork_Z <- linear_reconstruction(newyork)[[3]]

ggplot(newyork[[1]], aes(x=number))+
  geom_line(aes(y=newyork_Z))+
  labs(x="Biweek", y="Z Dynamics")
```
\

When local shifts in the mean of the Z dynamics are observed, such as those in Figure \ref{fig:Zlinear}, it indicates a nonconstant reporting rate. Following the work of Finkenstadt and Grenfell [@SRgrenfell], I addressed the issue of time-varying reporting rate by performing local linear regression with gaussian smoothers. To give an overview before I break down the process in detail, I split the data into overlapping chunks (or neighborhoods) centered around each $\{X_{i},...,X_{N}\}$, then assigned a gaussian weight to each observation in each neighborhood. Then I performed $N$ weighted linear regressions using the previously defined gaussian weights, and derived the value of $\rho$ for each time point by pulling the slope from each of these weighted linear regressions. 
...
To pull this together with an example, if I were to look at the datapoint $x_{10}$ (the sum of the cases from the first 10 biweeks), I would first create a vector of length $m$ of the $x_{t}$'s nearest to $x_{10}$. I would then apply the weight function $w_{i}(x_{10})=\frac{K(\frac{x_{10}-x_{i}}{h})}{\sum_{i=1}^{m}K(\frac{x_{10}-x_{i}}{h})}$ to each of the $x_{t}$'s in the neighborhood of $x_{10}$. 
