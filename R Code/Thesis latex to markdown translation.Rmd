...
```{r LocalReg, echo=FALSE, fig.cap="Z dynamics from local linear regression versus global linear regression taken from New York data"}
ggplot(newyork, aes(x=number))+
  geom_line(aes(y=newyork$Z_linear))+
  labs(x="Biweek", y="Z Dynamics (Linear Regression")

ggplot(newyork, aes(x=number))+
  geom_line(aes(y=newyork$Z_gaus))+
  labs(x="Biweek", y="Z Dynamics (Local Regression)")
```


Figure \ref[fig:LocalReg] above 
