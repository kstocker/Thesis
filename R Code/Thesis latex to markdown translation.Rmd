...
Sim2 <- function(beta, city, s_prop, i_prop, plot, title) {
...
choose_initials <- function(city, betas, bayes){
  ...
  mse <- function(x,y){
    sim <- Sim2(beta=rep_len(beta_est, length.out = 546), city=city, 
               s_prop=x, i_prop=y, plot=FALSE, title="hi")
...
validate_model <- function(city, betas, bayes, accuracy, plot_title){
  ...
  sim_plot <- Sim2(beta=rep_len(accuracy$beta_estimate, length.out=546), city=city, 
                  s_prop = s_prop, i_prop = i_prop, plot=TRUE, 
                  title=plot_title)[[2]]
...
To complete the reconstruction process, the infected dynamics are obtained by multiplying $\rho_{t}$ and the reported cases. 

At this point, the data is fully reconstructed and can be passed through Bayes without any further modification. As previously discussed, the Bayes output will include posterior distributions from all 26 $\beta_{t}$'s as well as for $\bar{S}$, the mean number of susceptibles. 
...
To this end, I simulated from my model for each of the cities I studied. However, before I could do that I needed to select point estimates for each $\beta_{t}$ and for the initial proportion of susceptibles and infecteds ($S_{0}$ and $I_{0}$). I began by using the median from the posterior distrubution of each $\beta_{t}$, and taking the initial proportion of susceptible and infected individuals from the first time point of my reconstructed susceptible and infected dynamics. However, the simulations are highly influenced by small deviations in the initial proportion of susceptibles and infecteds. For this reason, I decided to choose my initial proportion of susceptibles and infecteds by simulating from a range of proportions and choosing the initial conditions that yielded the lowest mean square error when compared with the incidence data from [@tycho]. I simulated with initial susceptible proportions between 0.025 and 0.05 and initial infected proportions between 0 and 0.001. I chose these values based on the work of Bjornstad et al [@BjornstadGrenfellMeasles]. Figure \ref{validate} shows the successful forward simulation using the initial conditions and transmission rates taken from this procedure. 

```{r Load Validation Info, include=FALSE}
...
```{r Validation, echo=FALSE, fig.cap="Plots show the successful forward simulation from the deterministic skeleton of the TSIR model parameterized with city-level data (in blue)  versus the incidence data taken from tycho (in red). Simulations were started from  the initial conditions obtained by minimizing the mean square error and were not fed any additional information from the incidence data once the simulation begun. Simulations were carried out for 20 years. \\label{validate}"}


ny_plot <- Sim2(beta=rep_len(ny_accuracy$beta_estimate, length.out = 546), 
                city=newyork, s_prop=ny_initials[[3]][2], 
                i_prop=ny_initials[[3]][1], plot=TRUE, title="New York")[[2]] 

chicago_plot <- Sim2(beta=rep_len(chicago_accuracy$beta_estimate, length.out = 546),
                     city=chicago, s_prop=chicago_initials[[3]][2], 
                     i_prop=chicago_initials[[3]][1], plot=TRUE, 
                     title="Chicago")[[2]]

indianapolis_plot <- Sim2(beta=rep_len(indianapolis_accuracy$beta_estimate, 
                                       length.out = 546), city=indianapolis, s_prop 
                          = indianapolis_initials[[3]][2], i_prop =
                            indianapolis_initials[[3]][1], plot=TRUE, 
                          title="Indianapolis")[[2]]
                    

plot_grid(ny_plot, chicago_plot, indianapolis_plot, ncol = 1, nrow = 3)
```
