...
read_in_city <- function(city){
  ...
  for(i in 1:nrow(df)){
    if(is.na(df$cases[i])==TRUE){
      df$cases[i] <- (df$cases[i-1]+df$cases[i+1])/2
    }
  }
...
#Chicago Reconstruction: Doesn't work because of a single NA
chicago <- read_in_city("CHICAGO")[[1]]
chicago_betas <- read_in_city("CHICAGO")[[2]]

#scale_optim_chicago <- optim_bandwidth(chicago)
...
scale_optim_phil <- optim_bandwidth(phil)
