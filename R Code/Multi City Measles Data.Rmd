...
read_in_city_na <- function(city_){
  ...
  nas <- which(is.na(df$cases))
  
  for(i in nas){
    if(i==1 &((i+1) %in% nas)==FALSE){
      df$cases[i] <- df$cases[i+1]
    }
     if(i==1 &((i+1) %in% nas)==TRUE){
      df$cases[i] <- df$cases[i+1]
    }
    if(i!=1 & ((i-1) %in% nas)==FALSE{
      df$cases[i] <- (df$cases[i-1]+df$cases[i+2])/2
...
```{r}
save(chicago_bayes, file="Chicago_Bayes")
