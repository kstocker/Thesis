...
Sim <- function(beta, city, s_prop, plot) {
  i.now <- city$cases[1] * city$rho[1]
...
forward_sim <- function(city, betas, bayes){
  beta_est <- accuracy_check(betas, bayes)$beta_estimate
  sbar_range <- accuracy_check(betas, bayes)$sbar_estimate
  
  (ny_sbar_range[1] + newyork$Z_gaus[1])/newyork$pop[1]
  (ny_sbar_range[3] + newyork$Z_gaus[1])/newyork$pop[1]
  
  _sim <- Sim(beta=rep_len(beta_est, length.out = 546), city=city, s_prop = s_prop, plot=FALSE)
  
 mean_square_error <- sum((_sim$Cases/city$pop-city$cases/city$pop)^2)
   
 }
}

NY_sim <- Sim(beta=rep_len(ny_betas_est, length.out = 546), city=newyork, s_prop=0.035, plot=TRUE)

NY_sim[[2]]
ggplot(NY_sim[[1]], aes(x=seq(1,length(I), 1)))+
  geom_line(aes(y=newyork$cases, col="Data"))+
  geom_line(aes(y=Cases, col="Sim Cases"))

sum((NY_sim[[1]]$Cases/newyork$pop[1]-newyork$cases/newyork$pop[1])^2)
```
